...
public class GameRoom extends JFrame implements ActionListener, MouseListener {
	...
	public void initGUI() {
		...
		allarea.setImageFile(new File("pics\\大厅.jpg"));
		add(allarea);
	...
	public void inittablearea() {
		...
		for (int i = 0; i < 9; i++) {
			...
			seat[i][2].setBounds(105, 55, 40, 40);// 右
			seat[i][3].setBounds(55, 105, 40, 40);// 右
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() instanceof Seat && !startflag) {
			...
			if (!lb.state) {
				...
				//启动游戏
				//System.out.println(playerID+serverIP+lb.tablenum+lb.seatnum);
