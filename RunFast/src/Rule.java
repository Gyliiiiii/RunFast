...
public class Rule {
	...
	public static boolean Isregular(String[] x, String[] y) {
		...
		if (lengthX != lengthY && lengthX != 4&& lengthX != 2)
			return false;
		...
		if ((lengthX == 4 && lengthY != 4 && duizi(dataX) != 0 )||(lengthY==2&&wangzha(dataY)!=0)) {
			Playsound.play("audio/Man/zhadan.wav");
			return true;
		}
		if (lengthX == 2 && lengthY != 2 && wangzha(dataX) != 0) {
			Playsound.play("audio/Man/wangzha.wav");
			return true;
		}
		...
		case 3:
			...
			break;
		case 4:
			if (duizi(dataX)>0 && duizi(dataX) > duizi(dataY))
				result = true;
			if(sandaiyi(dataX) > 0 && sandaiyi(dataX) > sandaiyi(dataY))
				result = true;
			break;
		case 5:
	 ...
	 * 王炸
	 */
	private static int wangzha(int[] x) {
	 ...
	 * 三带一
	 */
	private static int sandaiyi(int[] x) {
		if (x[0] == x[1] && x[1] == x[2])
			return x[0];
