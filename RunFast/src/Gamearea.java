...
public class Gamearea extends JFrame implements MouseListener, Runnable,
	...
	JButton start, chupai, buchu,buqiang,qiangdizhu; // 开始、出牌、提示按钮
	...
	Card[][] cardfront = new Card[4][20]; // player出的牌

	Startanimation startanimation;

	Timepass timepass;

	boolean[] cardclick = new boolean[21]; // 是否已点击牌

	boolean[] cardsend = new boolean[21]; // 牌是否出了

	int firstchupai = -1;// 先出牌的seat号

	String[] CARDS = new String[20]; // 用于接收17张牌

	Point[] endpos = new Point[21];

	Point[] startpos = new Point[21];
	...
	String[] cardinfo = new String[17]; // 记录上一个出牌player的牌信息

	String[] mycard = new String[17]; // 记录自己出牌信息

	String[] myoldcard = new String[17]; // 记录自己前一论出的牌
	...
	public void initGUI() {
		setTitle("RunFast");
		setIconImage(new ImageIcon("pics\\rush.jpg").getImage());
		setSize(940, 700);
		...
		/*buqiang = new JButton();
		buqiang.setBounds(450, 450, 55, 22);
		buqiang.setIcon(new ImageIcon("pics\\button\\提示.png"));
		buqiang.setContentAreaFilled(false);
		buqiang.setBorderPainted(false);
		buqiang.addMouseListener((MouseListener) this);
		qiangdizhu = new JButton();
		qiangdizhu.setBounds(350, 450, 55, 22);
		qiangdizhu.setIcon(new ImageIcon("pics\\button\\提示x.png"));
		qiangdizhu.setContentAreaFilled(false);
		qiangdizhu.setBorderPainted(false);
		qiangdizhu.addMouseListener((MouseListener) this);*/
		...
		*/
		...
		gamearea.add(chupai);
		gamearea.add(buchu);
		/*gamearea.add(buqiang);
		gamearea.add(qiangdizhu);*/
		...
		for (int i = 0; i < 4; i++) {
			...
			rolefropic[i].setBounds(-5, -20, 170, 210);
			ready[i] = new JLabel();
			ready[i].setBounds(65, 175, 100, 40);
		...
		role[seatnum].setBounds(300, 350, 170, 210);
		role[(seatnum + 1) % 4].setBounds(500, 190, 170, 210);
		role[(seatnum + 2) % 4].setBounds(100, 190, 170, 210);
		role[(seatnum + 3) % 4].setBounds(300, 50, 170, 210);
		...
		reready[(seatnum + 2) % 4].setBounds(180, 250, 66, 34);
		reready[(seatnum + 3) % 4].setBounds(330, 120, 66, 34);
		...
		clock[(seatnum + 2) % 4].setBounds(45, 140, 40, 43);
		clock[(seatnum + 3) % 4].setBounds(345, 50, 40, 43);
		...
		for (int i = 0; i < 13; i++) {
			cardfront[(seatnum + 1) % 4][16 - i] = new Card();
			cardfront[(seatnum + 1) % 4][16 - i].setVisible(false);
			cardfront[(seatnum + 1) % 4][16 - i].setBounds(570 - i * 15, 250,
					65, 100);
			gamearea.add(cardfront[(seatnum + 1) % 4][16 - i]);
			cardfront[(seatnum + 1) % 4][16 - i].addMouseListener(this);
		}

		for (int i = 0; i < 13; i++) {
			cardfront[(seatnum + 2) % 4][16 - i] = new Card();
			cardfront[(seatnum + 2) % 4][16 - i].setVisible(false);
			cardfront[(seatnum + 2) % 4][16 - i].setBounds(440 - i * 15, 140,
					65, 100);
			gamearea.add(cardfront[(seatnum + 2) % 4][16 - i]);
			cardfront[(seatnum + 2) % 4][16 - i].addMouseListener(this);
		}

		for (int i = 0; i < 13; i++) {
			cardfront[(seatnum + 3) % 4][16 - i] = new Card();
			cardfront[(seatnum + 3) % 4][16 - i].setVisible(false);
			cardfront[(seatnum + 3) % 4][16 - i].setBounds(440 - i * 15, 140,
					65, 100);
			gamearea.add(cardfront[(seatnum + 3) % 4][16 - i]);
			cardfront[(seatnum + 3) % 4][16 - i].addMouseListener(this);
		}
		...
		buchuarea[(seatnum + 2) % 4].setBounds(180, 250, 90, 40);
		buchuarea[(seatnum + 3) % 4].setBounds(330, 120, 90, 40);
	...
	/*	for (int i = 0; i < 3; i++) {
			threeCard[i] = new Card();
			threeCard[i].setVisible(true);
			threeCard[i].setBounds(260+i*70,40,65,100);
			threeCard[i].setImage("pics//cardbg.png");
			gamearea.add(threeCard[i]);
	...
	public void initcard() {
		...
		for (int i = num; i >= 1; i--) {
			...
			card[i].setImage("pics//cards//" + CARDS[num - i].charAt(0)
					+ CARDS[num - i].substring(1, CARDS[num - i].length())
					+ ".gif");
			card[i].priority = Integer.parseInt(CARDS[num - i].substring(1,
					CARDS[num - i].length())); // 获得牌的大小
			card[i].cardtype = CARDS[num - i].charAt(0) + ""; // 获得牌的种类
	...
	public void qiangdizhu() {
		for(int i=0;i<13;i++)
		{
			if (card[i].priority==3&&card[i].cardtype=="c") 
			{
				
				try {
					os.writeUTF("qiangdizhu");
					os.writeUTF(seatnum+ "");
					//System.out.println("玩家抢地主");
				} catch (IOException e1) {
				}
			}
		}
		...
		if (e.getSource() instanceof JButton) {
			...
			if (e.getSource() == buchu) {
				...
				Playsound.play("audio//Man//buyao"
						+ (new Random().nextInt(100) % 3 + 1) + ".wav");
				Cardreturn();
	...
	class Timepass extends Thread {
		public void run() {
			while (startflag) {
				clock[firstchupai % 3].setVisible(true);
				Isnext = false;
				for (int i = 30; i > 0; i--) {
					if (startflag == false)
						return;
					...
					if (Isnext == true) {
						clock[firstchupai % 3].setVisible(false);
						firstchupai++;
		...
		public void run() {
			...
			if (Isfirst == true) {
				for (int i = 0; i < 100; i++) {
					...
					role[seatnum].setBounds(300 - 2 * i, 350 + i, 170, 210);
					role[(seatnum + 1) % 4].setBounds(500 + i, 190, 170, 210);
					role[(seatnum + 2) % 4].setBounds(100 - i, 190, 170, 210);
					role[(seatnum + 3) % 4].setBounds(500 - 2 * i, -100 + i, 170, 210);
			...
			try {
				...
				if (s.equals("oneplayerexit")) {
					...
					if (startflag == true) { // 如果本局已经开始
						...
						cardinfo = new String[17];
						mycard = new String[17];
						myoldcard = new String[17];
						for (int i = 0; i < 13; i++) {
							cardinfo[i] = "";
							mycard[i] = "";
							myoldcard[i] = "";
				...
				if (s.equals("gamestart")) {
					...
					/*String card1=is.readUTF();
					String card2=is.readUTF();
					String card3=is.readUTF();
					String card4=is.readUTF();
					
					threeCard[0].setImage("pics//cards//" +card1+".gif");
					threeCard[1].setImage("pics//cards//" +card2+".gif");
					threeCard[2].setImage("pics//cards//" +card3+".gif");
					threeCard[3].setImage("pics//cards//" +card4+".gif");*/
